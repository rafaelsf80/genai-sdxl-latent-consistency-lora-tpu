# Sample inference for SDXL in TPU v5e using Latent Consistency LoRAs with the new FlaxLCMScheduler
...
import time
from diffusers import FlaxStableDiffusionXLPipeline, FlaxLCMScheduler, StableDiffusionXLPipeline
...
pipeline_old = StableDiffusionXLPipeline.from_pretrained(
...
pipeline_old.scheduler = FlaxLCMScheduler.from_config(pipeline_old.scheduler.config)


pipeline_old.fuse_lora(lora_scale=0.7)
pipeline_old.save_pretrained('sd_lora_model', safe_serialization=False)
pipeline_old.scheduler.save_pretrained('sd_lora_model')
...
pipeline, params = FlaxStableDiffusionXLPipeline.from_pretrained(
    "sd_lora_model", split_head_dim=True, from_pt=True
)

scheduler_state = params.pop("scheduler")
params = jax.tree_util.tree_map(lambda x: x.astype(jnp.bfloat16), params)
params["scheduler"] = scheduler_state
pipeline.scheduler.set_timesteps(params["scheduler"], num_inference_steps=1000)
